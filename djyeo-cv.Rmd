...
cv_entries %>% 
  ...
  mutate(what = ifelse(is.na(year_begin), paste0(what, "."), paste0(what, ", ", "<span style= 'font-weight: 500'>", where, "</span>")),
         what = gsub("NA", "", what),
         what = gsub(", $", "", what),
...
```{r}
kable(conf_tbl[seq(14, nrow(conf_tbl)), ])
```
...
kable(regional_tbl[seq(1, nrow(regional_tbl)), ])
```
